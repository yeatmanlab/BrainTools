...
def plotsig3(times,nReps,X,task1, task2, task3, subject, p_value=0.05):
    ...
    for i in range(0,X.shape[0]-t_window):
        ...
        X2 = np.mean(X[i:i+t_window,subject,task3], axis=0)
    ...
    for i in range(0,len(times)-t_window):
        if result[i]:
            if times[i] > 0.2 and k == 0:
                ...
                plt.text(times[i],2.7,np.str(times[i]))
    ...
    p = np.empty((X.shape[0],2))
    for i in range(0,X.shape[0]-t_window):
        X1 = np.mean(X[i:i+t_window,subject,task2], axis=0)
        X2 = np.mean(X[i:i+t_window,subject,task3], axis=0)
        true_diff = np.mean(X1) - np.mean(X2)
    ...
    for i in range(0,len(times)-t_window):
        if result[i]:
            if times[i] > 0.2 and k == 0:
                ...
                plt.plot([times[i],times[i]],[0,2.7],'--',color=[0,0,0])
                plt.text(times[i],2.7,np.str(times[i]))
            plt.plot(times[i],result[i]*0.3,'o',markerfacecolor=c_table[1],markeredgecolor=c_table[3])
