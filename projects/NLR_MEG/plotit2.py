...
def plotit2(times, M, errM, task1, task2, yMin=-1, yMax=5, subject = 'all'):
    plt.figure()
    plt.clf()    
...
#    plt.hold(True)
    ...
    plt.plot(times, M[:,task2],'-',color=[0,0,0],label='Med noise')
    plt.fill_between(times, M[:,task2]-errM[:,task2], M[:,task2]+errM[:,task2], facecolor=[0,0,0], alpha=0.2, edgecolor='none')
