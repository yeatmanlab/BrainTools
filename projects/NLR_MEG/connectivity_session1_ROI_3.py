...
#                                             subjects_dir=fs_dir)
#        stc_fsaverage = stc.morph_precomputed('fsaverage', fs_vertices, morph_mat, subject_from=subs[n])
        
        # Restrict the source estimate to the label in the left auditory cortex.
#        stc_label = stc_fsaverage.in_label(STSdp_label_lh)
        ...
        indices = mne.connectivity.seed_target_indices([seed_idx], np.arange(n_sources))
        ...
        con, freqs, times, n_epochs, n_tapers = mne.connectivity.spectral_connectivity(
            stcs, method='pli', mode='multitaper', indices=indices,
            sfreq=sfreq, fmin=fmin, fmax=fmax, faverage=False, n_jobs=18, mt_adaptive=True)

        tmin = np.mean(freqs[0])
        tstep = np.mean(freqs[1]) - tmin
        coh_stc = mne.SourceEstimate(con, vertices=stc.vertices, tmin=1e-3 * tmin,
                                     tstep=1e-3 * tstep, subject=subs[n])
        
        # Now we can visualize the coherence using the plot method.
        brain = coh_stc.plot(hemi='lh', subject = subs[n], views = ['lat','ven'],
                             time_label='Coherence %0.3f Hz',
                             subjects_dir=fs_dir,
                             initial_time = coh_stc.tmin + 0*coh_stc.tstep,
                             clim=dict(kind='value', lims=(0.25, 0.4, 0.65)))
        brain.add_label(s_label, borders=True, color=c_table[0])
        
        os.chdir('..')
        brain.save_movie('DotTask_LowNoise_900_PLI_lh.mp4',time_dilation = 40.0,framerate = 30)
