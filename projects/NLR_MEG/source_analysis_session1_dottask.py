...
brain3_1 = temp3.plot(hemi='lh', subjects_dir=fs_dir, views = ['lat'], #views=['lat','ven','med'], #transparent = True,
          initial_time=0.15, clim=dict(kind='value', lims=[1.5, 1.72, np.max(temp3.data[:,:])])) #pos_lims=[0, 4, 4.5] #np.max(temp3.data[:,:])]))
...
for iSub in np.arange(0,len(poor_readers)):
    ...
    plt.subplot(1,2,1)
    plt.hold(True)
    plt.plot(times, np.mean(X2[ventral_vertices,:,poor_readers[iSub],0],axis=0), '-', color=c_table[5])
    plt.plot(times, np.mean(X2[ventral_vertices,:,poor_readers[iSub],1],axis=0), '-', color=c_table[3])
    plt.plot(times, np.mean(X2[ventral_vertices,:,poor_readers[iSub],3],axis=0), '-', color=c_table[1])
#    plt.plot([0.1, 0.1],[0, 8],'-',color='k')
    plt.title(subs2[poor_readers[iSub]])
    plt.subplot(1,2,2)
    plt.hold(True)
    plt.plot(times, np.mean(X2[ventral_vertices,:,poor_readers[iSub],5],axis=0), '-', color=c_table[5])
    plt.plot(times, np.mean(X2[ventral_vertices,:,poor_readers[iSub],6],axis=0), '-', color=c_table[3])
    plt.plot(times, np.mean(X2[ventral_vertices,:,poor_readers[iSub],8],axis=0), '-', color=c_table[1])
#    plt.plot([0.1, 0.1],[0, 8],'-',color='k')
    plt.title(subs2[poor_readers[iSub]])
#%% 
""" Lexical task: V1 """
...
plt.plot(times, M[:,5],'-',color=c_table[5],label='High contrast')
plt.fill_between(times, M[:,5]-errM[:,5], M[:,5]+errM[:,5], facecolor=c_table[5], alpha=0.2, edgecolor='none')

plt.plot(times, M[:,7],'-',color=c_table[7],label='Low contrast')
plt.fill_between(times, M[:,7]-errM[:,7], M[:,7]+errM[:,7], facecolor=c_table[7], alpha=0.2, edgecolor='none')
...
plt.plot(times, M1[:,5],'-',color=c_table[5],label='High contrast')
plt.fill_between(times, M1[:,5]-errM1[:,5], M1[:,5]+errM1[:,5], facecolor=c_table[5], alpha=0.2, edgecolor='none')

plt.plot(times, M1[:,7],'-',color=c_table[7],label='Low contrast')
plt.fill_between(times, M1[:,7]-errM1[:,7], M1[:,7]+errM1[:,7], facecolor=c_table[7], alpha=0.2, edgecolor='none')
...
plt.plot(times, M2[:,5],'-',color=c_table[5],label='High contrast')
plt.fill_between(times, M2[:,5]-errM2[:,5], M2[:,5]+errM2[:,5], facecolor=c_table[5], alpha=0.2, edgecolor='none')

plt.plot(times, M2[:,7],'-',color=c_table[7],label='Low contrast')
plt.fill_between(times, M2[:,7]-errM2[:,7], M2[:,7]+errM2[:,7], facecolor=c_table[7], alpha=0.2, edgecolor='none')
